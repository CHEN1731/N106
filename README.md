...
3. **Productivity & Summary Dashboard (director & team) — the same web app at
   ...
   by-stage charts), **ERSS** (DW/BP/BT/CW KPI cards + Machine Status & Element Lifecycle —
   the detected machine data), **Excavation** (the Tunnel/FB soil-volume tracker),
   **Reinforced Concrete** (concrete m³ + RC activities), and **All Activities** (the
   full merged-activities breakdown, kept separate from the ERSS machine data). The
   **From/To date
